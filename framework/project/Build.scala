...
object DeliteBuild extends Build {
  ...
  val virtBuildSettingsBase = Defaults.defaultSettings ++ Seq(
    ...
    libraryDependencies += "jfree" % "jcommon" % "1.0.16",
		...
		libraryDependencies += "com.lowagie" % "itext" % "2.1.5" intransitive(),
		//libraryDependencies += "default" % "swing_2.10" % "0.1-SNAPSHOT",
  ...
  lazy val dsls = Project("dsls", file("dsls"), settings = virtBuildSettings) aggregate(optila, optiml, optiql, optimesh, optigraph, opticvx, optisdr) 
  ...
  lazy val optisdrApps = Project("optisdr-apps", file("apps/optisdr"), settings = virtBuildSettings) dependsOn(optisdr)
