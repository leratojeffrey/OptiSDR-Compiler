...
object DeliteTaskGraph {
  ...
  def processCommon(op: Map[Any, Any], opType: String)(implicit graph: DeliteTaskGraph) {
    ...
    val (fileName, line, opName) = getFieldMapOption(op, "sourceContext") match {
      ...
      case Some(sourceContext) =>
		...
		graph.optisdrKernelNames(graph.kidx) = opName
		graph.optisdropnames+=(id->opName)
    //print(graph.optisdrKernelNames(graph.kidx))
    //println(" is the Same as "+opName)
		graph.kidx = graph.kidx + 1		
  ...
  def newGraph(implicit graph: DeliteTaskGraph) = {
    ...
    newGraph.optisdrKernelNames = graph.optisdrKernelNames // TODO:
  ...
  def processWhileTask(op: Map[Any, Any])(implicit graph: DeliteTaskGraph) {
    ...
    //var internalOps = (predGraph.ops ++ bodyGraph.ops)
    var internalOps = (predGraph.ops ++ bodyGraph.ops).toSet
    ...
    whileOp.antiDeps = whileAntiDeps
		//
    // TODO: OptiSDR Stuff Here   
		//println("[OPISDR-INFO]$ Processing While Loop SubOps for OptiSDR Optimization...")
		parseOps2(getFieldList(op, "bodyOps"))
		parseOps2(getFieldList(op, "condOps"))
		//
		//
    if (predValue == "") {
  ...
  // OptiSDR Stuff 2:
	//
  def parseOps2(ops: List[Any])(implicit graph: DeliteTaskGraph) {
    for(_op <- ops) {
      val op = _op.asInstanceOf[Map[Any, Any]]
      val opType = getFieldString(op, "type") 
      opType match {
        case "SingleTask" => processCommon2(op, "OP_Single")
        case "External" => processCommon2(op, "OP_External")
        case "Input" => processCommon2(op, "OP_FileReader")
        case "MultiLoop" => processCommon2(op, "OP_MultiLoop")
        case "Foreach" => processCommon2(op, "OP_Foreach")
        case "Conditional" => processIfThenElseTask(op)
        case "WhileLoop" => processWhileTask(op)
        case "Arguments" => processArgumentsTask(op)
        case "EOP" => processEOPTask(op) //end of program
	...
	def processCommon2(op: Map[Any, Any], opType: String)(implicit graph: DeliteTaskGraph)
	{
    val id = getFieldString(op, "kernelId")
    ...
    val (fileName, line, opName) = getFieldMapOption(op, "sourceContext") match {
      ...
      case Some(sourceContext) =>
		...
		graph.optisdrKernelNames(graph.kidx) = opName
		graph.optisdropnames+=(id->opName)
		graph.optisdrwhileops+=(id->opName) // This are while sub-ops, we need these for new CUFFT-based
		//whileopsid=whileopsid + 1
    //print(graph.optisdrKernelNames(graph.kidx))
    //println(" is the Same as "+opName)
		graph.kidx = graph.kidx + 1		
    //
...
class DeliteTaskGraph {
	var optisdrKernelNames: Array[Any] = new Array[Any](1000000)  	
  ...
  protected var _appName = ""
	...
	var optisdropnames = new HashMap[String, String]
	var optisdrwhileops = new HashMap[String, String]
  ...
  def inputs: Set[(OP_Input,String)] = _inputs.toSet[(String,OP_Input)].map(i=>Pair(i._2,i._1))
	...
	//def optisdrKernelNames: List[Any]
