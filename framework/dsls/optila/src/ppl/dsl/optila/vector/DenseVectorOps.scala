...
trait DenseVectorOps extends Variables {
  ...
  object DenseVector {
    ...
    def onesi(len: Rep[Int])(implicit ctx: SourceContext) = densevector_obj_onesi(len)
  ...
  def densevector_obj_onesi(len: Rep[Int])(implicit ctx: SourceContext): Rep[DenseVector[Int]]
...
trait DenseVectorOpsExp extends DenseVectorOps with DeliteCollectionOpsExp with DeliteStructsExp {
  ...
  def densevector_obj_onesi(len: Exp[Int])(implicit ctx: SourceContext) = reflectPure(DenseVectorObjectConst(len,unit(true),unit(1)))
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit ctx: SourceContext): Exp[A] = (e match {
    ...
    case Reflect(e@DenseVectorObjectFromUnliftedSeq(xs), u, es) => reflectMirrored(Reflect(new { override val original = Some(f,e) } with DenseVectorObjectFromUnliftedSeq(f(xs))(e.mA), mapOver(f,u), f(es)))(mtype(manifest[A]), ctx)
