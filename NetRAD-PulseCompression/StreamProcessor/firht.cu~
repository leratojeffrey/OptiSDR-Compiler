...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//
void dHilbertSP(vector<cuFloatComplex*> ddata, vector<cuFloatComplex*> hout, int dsize,int chunk, int ftpoint)
{
	//
	dim3 dimBlock, dimGrid;
    	int threadsPerBlock;//, blocksPerGrid;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//
void d2dHilbertSP(vector<cuFloatComplex*> ddata, vector<cuFloatComplex*> dout, int dsize,int chunk, int ftpoint)
{
	//
	dim3 dimBlock, dimGrid;
    	int threadsPerBlock;//, blocksPerGrid;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//
__host__ __device__ __inline__ void pad(cuFloatComplex* A,cuFloatComplex* B,int C1,int C2)
{
	#pragma unroll
	for(int i=C2; i<C1+C2;i++) // TODO: Make sure C1 <= C2 always
		B[i] = A[i];
}
//
__global__ void zeropad(cuFloatComplex* A,cuFloatComplex* B,int C1, int R,int C2) // Parallel Zero Padding Columns of a Matrix: sizeof(B) = (C2/C1 = 2) * sizeof(A)
{
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	if(tid < R)
		pad(A,B,C1,tid*C2);
...
//
//
__host__ __device__ __inline__ void pad(float* A,float* B,int C1,int C2)
{
	#pragma unroll
	for(int i=C2; i<C1+C2;i++) // TODO: Make sure C1 <= C2 always
		B[i] = A[i];
...
//
__global__ void zeropad(float* A,float* B,int C1, int R,int C2) // Parallel Zero Padding Columns of a Matrix: sizeof(B) = (C2/C1 = 2) * sizeof(A)
{
	int tid = threadIdx.x + blockIdx.x * blockDim.x;
	if(tid < R)
		pad(A,B,C1,tid*C2);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//
void XCorrSPd2h(vector<cuFloatComplex*> ddata, cuFloatComplex* refsig, vector<cuFloatComplex*> hout, int dsize,int chunk, int ftpoint)
{
	//
	dim3 dimBlock, dimGrid;
    	int threadsPerBlock, blocksPerGrid;
	...
	threadsPerBlock = dimBlock.x*dimBlock.y*dimBlock.z;
        // Set up grid
        // Here you will have to set up the variables: dimGrid and blocksPerGrid
	int grid1 = ceil(2*chunk/(float)threadsPerBlock); // For 1-D Grid
	//int grid1 = ceil(sqrt(numEls/(float)threadsPerBlock)); // For 2-D Grid
	//int grid1 = ceil(cbrt(numEls/(float)threadsPerBlock)); // For 3-D Grid
	dimGrid.x = grid1;
	//dimGrid.y = grid1; // uncomment for 2-D grid
	...
	int n[1] = {2*ftpoint};
	//
	cuFloatComplex *drefsig,*dftrefsig/*,dzprefsig*/;
	cudaMalloc((void**)&drefsig,2*chunk*sizeof(cuFloatComplex)); // We want to zeropad this...
	cudaMalloc((void**)&dftrefsig,2*chunk*sizeof(cuFloatComplex));
	//cudaMalloc((void**)&dzprefsig,2*chunk*sizeof(cuFloatComplex));
	//cudaHostRegister(refsig,chunk*sizeof(cuFloatComplex),cudaHostRegisterPortable);
	cudaMemcpy(drefsig,refsig,2*chunk*sizeof(cuFloatComplex),cudaMemcpyHostToDevice);
	//cudaMalloc((void**)&drout,chunk*sizeof(cuFloatComplex));
	//
	vector<cuFloatComplex*> dzpout,dout,drout;
	dzpout.resize(dsize);
	dout.resize(dsize);
	drout.resize(dsize);
	//
	...
	for(int i = 0; i<dsize; i++)
	{
		...
		//cufftPlanMany(&plans[i],1,n,NULL,1,SIG_LEN,NULL,1,SIG_LEN,CUFFT_C2C,N_SIGS);
		res = cufftPlanMany(&plans[i], 1, n,
    		NULL, 1,2*ftpoint, //advanced data layout, NULL shuts it off
    		NULL, 1,2*ftpoint, //advanced data layout, NULL shuts it off
    		CUFFT_C2C,chunk/ftpoint);
   		if (res != CUFFT_SUCCESS) {printf("[SDR_DSL_INFO]$ Plan create fail.\n");}
		//
	...
	{
		//cudaHostRegister(hdata[i],chunk*sizeof(cuFloatComplex),cudaHostRegisterPortable);
		cudaHostRegister(hout[i],2*chunk*sizeof(cuFloatComplex),cudaHostRegisterPortable);
		//cudaStreamCreate(&optisdr_streams[i]);
		//cudaMemcpyAsync(ddata[i],hdata[i],chunk*sizeof(cuFloatComplex),cudaMemcpyHostToDevice,optisdr_streams[i]);
	}
	//
	for(int i = 0;i<dsize;i++)
	{
		cudaMalloc((void**)&dzpout[i],2*chunk*sizeof(cuFloatComplex)); // Zero-Padding out is twice size of input
		cudaMalloc((void**)&dout[i],2*chunk*sizeof(cuFloatComplex));
		cudaMalloc((void**)&drout[i],2*chunk*sizeof(cuFloatComplex));
		//cudaMemcpyAsync(ddata[i],hdata[i],chunk*sizeof(cuFloatComplex),cudaMemcpyHostToDevice,optisdr_streams[i]);
		//
	}
	//
	// drefsig
	//zeropad<<<chunk/1024,1024>>>(ddata2[0][0],dout2[0][0],2048,chunk/2048,4096);
	//zeropad<<<dimGrid,dimBlock,0,optisdr_streams[0]>>>(drefsig,dzprefsig,ftpoint,chunk/ftpoint,2*ftpoint);
	//	
	res = cufftExecC2C(plans[0],drefsig,dftrefsig,CUFFT_FORWARD);
	if (res != CUFFT_SUCCESS) {printf("[SDR_DSL_INFO]$ Forward transform fail.\n");}
	// Execution
	...
	{
		// Zero-Padding
		zeropad<<<dimGrid,dimBlock,0,optisdr_streams[i]>>>(ddata[i],dzpout[i],ftpoint,chunk/ftpoint,2*ftpoint);
		// FFT
		res = cufftExecC2C(plans[i],dzpout[i],dout[i],CUFFT_FORWARD);
		if (res != CUFFT_SUCCESS) {printf("[SDR_DSL_INFO]$ Forward transform fail.\n");}
		// Multiplication with reference signal: TODO: ???
		complexvector_multiply1d1d<<<dimGrid,dimBlock,0,optisdr_streams[i]>>>(dout[i],dftrefsig,drout[i],2*chunk);
		// iFFT
		res = cufftExecC2C(plans[i],drout[i],ddata[i],CUFFT_INVERSE);
		if (res != CUFFT_SUCCESS) {printf("[SDR_DSL_INFO]$ Inverse transform fail.\n");}
		//
		// Scale iFFT output
        	optisdrifftscale<<<dimGrid,dimBlock,0,optisdr_streams[i]>>>(ddata[i],dout[i],2*ftpoint,2*chunk);
		// Copy data back to Host
		cudaMemcpyAsync(hout[i],dout[i],2*chunk*sizeof(cuFloatComplex),cudaMemcpyDeviceToHost,optisdr_streams[i]);
		cudaStreamSynchronize(optisdr_streams[i]);
	}
	//
	// Releasing Computing Resources
...
//
void ZeroPadSP(vector<cuFloatComplex*> hdata, vector<cuFloatComplex*> hout, int dsize,int chunk, int ftpoint)
{
	//
	dim3 dimBlock, dimGrid;
    	int threadsPerBlock, blocksPerGrid;
	...
	{
		// Zero-Padding
		zeropad<<<dimGrid,dimBlock,0,optisdr_streams[i]>>>(ddata[i],dout[i],ftpoint,chunk/ftpoint,2*ftpoint);
		//
		// Copy data back to Host
		cudaMemcpyAsync(hout[i],dout[i],2*chunk*sizeof(cuFloatComplex),cudaMemcpyDeviceToHost,optisdr_streams[i]);
		cudaStreamSynchronize(optisdr_streams[i]);
...
//
void XCorrSPh2h(vector<cuFloatComplex*> hdata, cuFloatComplex* refsig, vector<cuFloatComplex*> hout, int dsize,int chunk, int ftpoint)
{
	//
	dim3 dimBlock, dimGrid;
    	int threadsPerBlock, blocksPerGrid;
	...
	//cudaMalloc((void**)&drout,chunk*sizeof(cuFloatComplex));
	//
	vector<cuFloatComplex*> ddata/*,dzpout*/,dout,drout;
	//dzpout.resize(dsize);
	dout.resize(dsize);
	drout.resize(dsize);
	ddata.resize(dsize);
	...
	for(int i = 0;i<dsize;i++)
	{
		cudaHostRegister(hdata[i],2*chunk*sizeof(cuFloatComplex),cudaHostRegisterPortable);
		cudaHostRegister(hout[i],2*chunk*sizeof(cuFloatComplex),cudaHostRegisterPortable);
		//cudaStreamCreate(&optisdr_streams[i]);
		//cudaMemcpyAsync(ddata[i],hdata[i],chunk*sizeof(cuFloatComplex),cudaMemcpyHostToDevice,optisdr_streams[i]);
	}
	...
	for(int i = 0;i<dsize;i++)
	{
		...
		cudaMalloc((void**)&dout[i],2*chunk*sizeof(cuFloatComplex));
		cudaMalloc((void**)&drout[i],2*chunk*sizeof(cuFloatComplex));
		cudaMalloc((void**)&ddata[i],2*chunk*sizeof(cuFloatComplex));
		cudaMemcpyAsync(ddata[i],hdata[i],2*chunk*sizeof(cuFloatComplex),cudaMemcpyHostToDevice,optisdr_streams[i]);
		//
	}
	//
	// drefsig
	...
	for(int i = 0; i<dsize; i++)
	{
		...
		complexvector_multiply1d1d<<<dimGrid,dimBlock,0,optisdr_streams[i]>>>(dout[i],dftrefsig,drout[i],2*chunk);
		// iFFT
		res = cufftExecC2C(plans[i],drout[i],dout[i],CUFFT_INVERSE);
		if (res != CUFFT_SUCCESS) {printf("[SDR_DSL_INFO]$ Inverse transform fail.\n");}
		//
		// Scale iFFT output
        	optisdrifftscale<<<dimGrid,dimBlock,0,optisdr_streams[i]>>>(dout[i],drout[i],2*ftpoint,2*chunk);
		// Copy data back to Host
		cudaMemcpyAsync(hout[i],drout[i],2*chunk*sizeof(cuFloatComplex),cudaMemcpyDeviceToHost,optisdr_streams[i]);
		cudaStreamSynchronize(optisdr_streams[i]);
	}
	//
	// Releasing Computing Resources
...
int main()
{
	...
	int ftpoint = 1*SIG_LEN; // TODO: Cater for Zero-Padding
	...
	//	
	cuFloatComplex *hdata,*hout2,*hout;
	hdata = getComplexSamples(vsNetRadSamples,0,dataLen);
	hout  = getComplexEmpty(2*dataLen);
	//
	//
	gettimeofday(&endt, 0);	
	...
	{
		int from = i*chunk;
		int to = i*chunk+chunk;
		cuFloatComplex *tmpin = getZeroPadded(getChunk(hdata,chunk,from,to),chunk,2*chunk,2048); // Append 2048 zeros to getChunk
		//
		append(hout,tmpin,2*dataLen,2*chunk,2*i*chunk);
		...
		//HilbertSP(hdata2[i],hout2[i],dsize,chunk,ftpoint);
		//XCorrSPh2h(hout2[i],hrefout,hout3[i],dsize,chunk,ftpoint);
	}
	//		
	...
	for(int i = subchunk/2; i<subchunk; i++)
	{
		...
		//ZeroPadSP(hout2[i],hout3[i],dsize,chunk,ftpoint);
		//(hout2[i],hrefout,hout3[i],dsize,chunk,ftpoint); // hout3 has twice hout2
	}
	//
	gettimeofday(&t2, 0);
	double time = (1000000.0*(t2.tv_sec-t1.tv_sec) + t2.tv_usec-t1.tv_usec)/1000000.0;
	...
	//
     	float *fftOutput = (float*)malloc(2*ftpoint*sizeof(float));
      	for (int i = 0; i < 1; i++)//subchunk
      	{
		cuFloatComplex* tmpout = hout;//hout2[0][0];//hout2[0][i];//TODO: Uncomment this after testing 
		//cuFloatComplex *tmpout = hdata2[0][i];
		for(int j = 0; j< 2*ftpoint; j++)
		{
	...
	//
	writeFileF("../data/zeropad.txt",fftOutput,2*ftpoint);
	// Clean-Up Boy
